...
> 语言无关的代码质量反模式，覆盖代码复用、抽象泄漏、参数膨胀等核心主题。适用于所有语言的 PR 审查。
...
6. [复制粘贴变种](#复制粘贴变种)
7. [空操作更新](#空操作更新)
8. [TOCTOU 竞争条件](#toctou-竞争条件)
9. [过度宽泛操作](#过度宽泛操作)
10. [冗余状态](#冗余状态)
11. [通用质量审查清单](#通用质量审查清单)

---

## 代码复用审查

接受新代码前，先在现有代码库中搜索可复用的工具函数。
...
```typescript
// ❌ Raw string 事件名——拼写错误不会报错
emitter.emit("userCreated", data);
emitter.on("usercreated", handler); // bug: 拼写错误
...
# ✅ 只读需要的内容
first_line = next(Path("log.txt").open())
```
